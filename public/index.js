...
let SLIDER = CANVAS.append('g')
  ...
  .attr(
    'transform',
    'translate(' + MARGIN.left + ',' + (MARGIN.top + CHART_HEIGHT) + ')'
  )
  .attr('height', SLIDER_HEIGHT)
  .attr('width', SLIDER_WIDTH);

let BORDER = CANVAS.append('rect')
...
function drawBubbles(data) {
  ...
  let bubble_group = CHART.selectAll('.bubble-group').data(data);
  ...
  let bubbles = bubble_group.append('circle').attr('class', 'bubble');
...
function handleMouseOver(d, i) {
  if (MOVING == false) {
    ...
    circle.attr('r', BUBBLE_RADIUS * 1.1).style('opacity', 0.9);
    group.select('text').style('font-size', '14.88px');
  }
}
...
function handleMouseOut(d, i) {
  if (MOVING == false) {
    ...
    group
      ...
      .style('opacity', 1.0);
    group.select('text').style('font-size', '12.4px');
  }
}
...
function showRelatedNews(topic) {
  ...
  rowData.append('p');
  rowData.append('p').html(function(d) {
...
function update() {
  ...
  SLIDER_LABEL.attr('x', numeric_value).text(FORMATDATE(date_value));
...
function add_new_data(year, month) {
  ...
  let center = [
    ...
    {
      cx: left_padding_three + 250 + 10 + space_three,
      cy: 100 - 10,
      topic: ''
    },
    {
      cx: left_padding_three + 400 + 80 + space_three,
      cy: 100 - 10,
      topic: ''
    },
...
function updateSliderAxis() {
  ...
  SLIDER_LABEL.attr('x', SLIDER_SCALE(STARTDATE)).text(FORMATDATE(STARTDATE));
  resetSlider();
}
...
function getSteps() {
  ...
  diff /= 60 * 60 * 24 * 7 * 4;
  return Math.abs(Math.round(diff)) <= 1 ? 1 : Math.abs(Math.round(diff));
}
